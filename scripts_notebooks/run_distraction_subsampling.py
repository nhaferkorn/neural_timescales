"""This script splits the encoding trials into high and low distraction trials. And subsamples the high distraction trials 
(since there are twice as many high distraction trials compared to low distraction trials). """
...
print(len(epochs_high))
print(len(epochs_low))

# balance epoch count by using equalize_event_count method
mne.epochs.equalize_epoch_counts([epochs_high, epochs_low], method='random')
...
for name, epoch in zip(epochs_names, epochs_list):
        ...
        # reshape 
        acf_low_reshaped = acf_trials.reshape(acf_trials.shape[0] * acf_trials.shape[1], acf_trials.shape[2])
        ...
        df = pd.DataFrame(acf_low_reshaped, columns = [f"tau_{name}", f"height_{name}", f"offset_{name}"])
        ...
        # save as csv file
        df.to_csv(path_or_buf = os.path.join(DERIV_DIR, 'timescales', 'acf_timescales', 'single_trials', 'distraction_subsampled', f'sub-{sub}_acf_params_{name}.csv'), sep = ',', header = True, index = False)
