...
subjects = ['sub-%d' % i for i in range(101, 162) if i != 105]
...
for subject in subjects:
    ...
    # construct filepath
    file_path_high = os.path.join(DERIV_DIR, 'timescales', 'acf_timescales', 'evoked', 'distraction', f'{subject}_acf_params_evoked_high.csv')
    file_path_low = os.path.join(DERIV_DIR, 'timescales', 'acf_timescales', 'evoked', 'distraction', f'{subject}_acf_params_evoked_low.csv')
...
# # fig.suptitle("Topographies Distraction Load", fontsize = 'xx-large')
# gs = GridSpec(1, 3)
...
# # ax.set_title('High Dist vs. Low Dist (n=53)', fontsize = 'x-large')
...
# # #     # initialize figure
# # #     fig, ax_topo = plt.subplots(1, 1)

# # #     # how to mark sensors in the cluster
# # #     mask_params = dict(marker='.', markerfacecolor='k', markersize=6)

# # #     # plot average test statistic and mark significant sensors
# # #     sel = mne.pick_types(infos[1], eeg=True)
# # #     info = mne.pick_info(infos[1], sel)
# # #     image, _ = mne.viz.plot_topomap(T_obs_map, info, ch_type='eeg', mask=mask, 
...
# # # fig.suptitle('Clusters and t-values')
# # # fig.savefig(os.path.join(DERIV_DIR, 'timescales', 'topos', f'topo_clusters_distraction_{date}.png'))