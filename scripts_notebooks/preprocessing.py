...
from src.eeg2bids import data_dir, bids_root

###################################################################################################
###################################################################################################

raw_file = data_dir / "eeg" / "102.bdf"

# right now this only loads the data for a single subject 
...
#%% Create events dictionary - The mapping right now is correct but what is missing are the following key value pairs: Start Encoding: "10", Begin Localizer / End Localizer: "30", and values from 1 to 8 (not sure what they mean)
...
# add elements to event_dict
# event_dict['Offset'] = 65536
...
#%% Set EEG reference
# set average of mastoid channels as reference
raw_mast_ref = raw.copy().set_eeg_reference(ref_channels = ['EXG5', "EXG6"])
...
raw_filtered = raw_mast_ref.filter(l_freq = 0.1, h_freq=None)
...
# okay, interesting - so apparently I need to specify this manually 
...
eog_events = mne.preprocessing.find_eog_events(raw, event_id=998, thresh = 200e-6)
